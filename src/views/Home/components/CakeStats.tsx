...
const CakeStats = () => {
  ...
  const faevPrice = usePriceCakeBusd()
  const circSupply = totalSupply ? totalSupply.minus(burnedBalance) : new BigNumber(0)
  const cakeSupply = getBalanceNumber(circSupply)
  const marketCap = faevPrice.times(circSupply)

  let faevPerBlock = 0
  if (farms && farms[0] && farms[0].faevPerBlock) {
    faevPerBlock = new BigNumber(farms[0].faevPerBlock).div(new BigNumber(10).pow(18)).toNumber()
  }
    ...
    <StyledCakeStats>
      <CardBody>
        <Heading size="lg" mb="24px">
          {TranslateString(534, 'Faev Stats')}
        </Heading>
        <RowHighlighted>
          <Text fontSize="14px">
            <strong>
        ...
        <Row>
          <Text fontSize="14px">
            {TranslateString(538, 'FAEV burnt by FaevSwaps')}{' '}
          ...
          <LinkExternal href="https://bscscan.com/token/0xa2f0B35B93f39F19B010999aaa28Bd5A77eb40f4?a=0x000000000000000000000000000000000000dead">
            <CardValue fontSize="14px" value={getBalanceNumber(burnedBalance)} decimals={0} />
          </LinkExternal>
        </Row>
        <Row>
          <Text fontSize="14px">{TranslateString(540, 'New FAEV/block')}</Text>
